...
void UB_Graphic2D_ClearSreenDMA(uint16_t c)
{
  ...
  col32=P_convert565to8888(c); // bug workaround (rgb565 bug)
...
// für PacMan
//--------------------------------------------------------------
void UB_Graphic2D_CopyImgRotate(UB_Image *img, DMA2D_Koord koord, uint8_t mode)
{
	uint32_t x,y;
	uint32_t xp,yp;
	uint32_t xs,ys,ws;
	...
	if(mode==0) {
      // right
		for(y=0;y<koord.source_h;y++) {
			UB_LCD_SetCursor2Draw(xp,yp+y);
			for(x=0;x<koord.source_w;x++) {
